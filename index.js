...
/* Get a numeric value from an item for price-based sorting.
   Boosting items are priced "per 1000 quantity" via `rate`, so use that
   instead of the (nonexistent) price field for those. */
const getPriceValue = (item) => {
  if (item.tag === "BOOSTING" || item.tag === "BOOST_SUB") {
    return Number(item.rate) || 0;
  }
  const num = parseFloat(String(getPrice(item)).replace(/[^\d.]/g, ""));
  return Number.isFinite(num) ? num : 0;
};

/* Returns a new array sorted by price, lowest to highest.
   Leaves the original array untouched. */
const sortByPriceAsc = (items = []) =>
  [...items].sort((a, b) => getPriceValue(a) - getPriceValue(b));
...
async function renderPromos() {
  ...
  await renderChunked(sortByPriceAsc(promoDatabase), promoCard, grid, 15);
}
...
async function renderExtraLayer() {
  ...
  await renderChunked(sortByPriceAsc(extraDatabase), promoCard, grid, 15);
}
...
function openVariants(platformName, parentCategory, description = "") {
  ...
  const products = sortByPriceAsc(productVariantsDB[platformName] || []);
...
function updateBoostCardPrice(card, rawQty) {
  if (!card) return;
...
function buyItem(card) {
  ...
  // For boosting items, make sure the price reflects exactly what's
  // currently typed in the quantity field (in case it changed but
  // didn't trigger a recalculation yet).
...
function handleClick(event) {
  ...
  if (qtyStepBtn) {
    ...
    if (card && input) {
      ...
      updateBoostCardPrice(card, current + delta);
    }
    return;
  }
...
function handleInput(event) {
  ...
  updateBoostCardPrice(card, qtyInput.value);
}
...
function handleFocusOut(event) {
  ...
  // If the field was left empty or invalid, restore it to a valid
  // quantity. Otherwise leave the user's exact typed value untouched.
  const qty = parseInt(qtyInput.value, 10);
  if (!Number.isFinite(qty) || qty < BOOST_MIN_QTY) {
    updateBoostCardPrice(card, BOOST_MIN_QTY);
  }
}
